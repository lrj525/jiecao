...
(function () {
    ...
    function setTabFocus(id) {
        if (!id) return;
        var onlinesearch = $G("onlinesearch");
        if (id == "online") {
            onlinesearch.style.display = "";
        } else {
            onlinesearch.style.display = "none";
        }
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                var authorizationData = getCookie("authorizationData");
                if (authorizationData) {
                    var jsonAuth = JSON.parse(authorizationData);
                    if (jsonAuth) {
                        header['subStationId'] = jsonAuth.subStationId;
                    }
                }
            });
    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            domUtils.on($G('imageList'), 'scroll', function(e){
                ...
                if (panel.scrollHeight - (panel.offsetHeight + panel.scrollTop) < 10) {
                    var onlinesearchkey = $G('onlinesearchkey').value;
                    _this.getImageData(onlinesearchkey);
                }
            });
            /* 选中图片 */
            ...
            domUtils.on($G('onlinesearchbtn'), 'click', function (e) { 
                var onlinesearchkey = $G('onlinesearchkey').value;
                _this.reset();
                _this.getImageData(onlinesearchkey);
            });
        ...
        initData: function () {
            ...
            var onlinesearchkey = $G('onlinesearchkey').value;
            /* 第一次拉取数据 */
            this.getImageData(onlinesearchkey);
        },
        /* 重置界面 */
        reset: function() {
            this.initContainer();
        ...
        /* 向后台拉取图片列表数据 */
        getImageData: function (searchkey) {
            var _this = this;
            var headers = {};
            var authorizationData= _this.getCookie("authorizationData");
            if (authorizationData) {
                var jsonAuth = JSON.parse(authorizationData);
                if (jsonAuth) {
                    headers = { subStationId: jsonAuth.subStationId };
                }
            }
        ...
        pushData: function (list) {
            var i, item, img, icon,title, _this = this,
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.setAttribute('src', urlPrefix + list[i].url + ".100x0" + list[i].extension + (list[i].url.indexOf('?') == -1 ? '?noCache=' : '&noCache=') + (+new Date()).toString(36));
                    img.setAttribute('_src', urlPrefix + list[i].url + ".720x0" + list[i].extension);
                    domUtils.addClass(icon, 'icon');
                    domUtils.addClass(title, 'title');
                    domUtils.addClass(title, 'ellipsis');
                    title.innerText = list[i].title;
                    title.setAttribute("title", list[i].title);
