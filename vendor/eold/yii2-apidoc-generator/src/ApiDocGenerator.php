...
class ApiDocGenerator extends Component {
    ...
    public function init()
    {
        ...
        //cdoc不为1时不创建api文档
            $createDoc=Yii::$app->request->get('cdoc',0);
            if($createDoc==0) return;
        //lrj:不加载已生成的api，用于重新生成，只适用于开发模式
    ...
    private function getFriendlyName($event)
    {
        ...
        $this->friendlyName = $friendlyName.".html";
    }
    ...
    {
        //lrj:检查已生成文件时，永远返回不存在
        return false;//isset($this->docs[$this->friendlyName]);
    }
