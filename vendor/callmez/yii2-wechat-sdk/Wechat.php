...
class Wechat extends Component
{
     ...
     * 导入code
     */
    const WECHAT_DEPOSIT_CARD_CODE_URL = '/card/code/deposit?';
    /**
     * 查询导入code数目
     */
    const WECHAT_GET_DEPOSIT_COUNT_URL = '/card/code/getdepositcount?';
    /**
     * 创建卡券
     */
    ...
    {
        return WeChatJsSDK::getJsApiTicket();
    }
    ...
    public function uploadCardLogo($file)
    {
        ...
        $json = exec('curl -F "buffer=@'.$file.'" "https://api.weixin.qq.com/cgi-bin/media/uploadimg?access_token='.$this->getAccessToken().'"');
    ...
    public function createCard($requestParams = [])
    {
        ...
        $url = static::WECHAT_BASE_URL.static::WECHAT_CREATE_CARD_URL . 'access_token=' . $this->getAccessToken();
     ...
     * @throws HttpException
     */
    public function importCode($requestParams)
    {
        if (empty($requestParams)) {
            return false;
        }
        ...
        $requestParams = json_encode($requestParams,JSON_UNESCAPED_UNICODE);
        $url = static::WECHAT_BASE_URL.static::WECHAT_DEPOSIT_CARD_CODE_URL . 'access_token=' . $this->getAccessToken();
        return json_decode(Helper::curlPost($url,$requestParams),true);
     ...
     * @return array
     */
    public function sendMessageCard($requestParams=[])
    {
        if(empty($requestParams)){
            return false;
        }
        $url = static::WECHAT_BASE_URL . static::WECHAT_CUSTOM_MESSAGE_SEND_URL . 'access_token=' . $this->getAccessToken();
        $requestParams = json_encode($requestParams,JSON_UNESCAPED_UNICODE);
        return json_decode(Helper::curlPost($url,$requestParams),true);
    }
     ...
     * @return array
     */
    public function countCode($requestParams=[])
    {
        if(empty($requestParams)){
            return false;
        }

        $url = static::WECHAT_BASE_URL . static::WECHAT_GET_DEPOSIT_COUNT_URL . 'access_token=' . $this->getAccessToken();
        $requestParams = json_encode($requestParams,JSON_UNESCAPED_UNICODE);
        return json_decode(Helper::curlPost($url,$requestParams),true);
    }
     ...
     * @throws HttpException
     */
    public function modifyCardStock($requestParams=[])
    {
        if (empty($requestParams)) {
            return false;
        }
        $url = static::WECHAT_BASE_URL . static::WECHAT_MODIFY_CARD_STOCK . 'access_token=' . $this->getAccessToken();
        $requestParams = json_encode($requestParams,JSON_UNESCAPED_UNICODE);
        return json_decode(Helper::curlPost($url,$requestParams),true);
