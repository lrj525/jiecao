...
class IDCardCheck
{
    ...
    //获取身份证出生日期
    public static function getDate($num){ 
        $num = strtolower($num);
        // 如果是15位身份证号
        if(strlen($num) == 15 ){
            ...
            $date1 = substr($dateNum,0,2);
            $date2 = substr($dateNum,2,2);
            $date3 = substr($dateNum,4,2);
          

            return '19'.$date1.'-'.$date2.'-'.$date3;
        }else{
            // 如果是18位身份证号
            // 出生年月（8位）
            $dateNum = substr($num,6,8);
    ...
    //获取身份证性别
    public static function getSex($num){ 
        $num = strtolower($num);
        // 如果是15位身份证号
        if(strlen($num) == 15 ){
            // 性别（3位）
